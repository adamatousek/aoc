...
do {
    ...
    if ( $intr_opcode == $I_OUT ) {
        ...
        if ($stage == 0 ) {
            ...
            @direction = ($direction[1], -$direction[0]) if $out == 0;
            @direction = (-$direction[1], $direction[0]) if $out == 1;
        ...
        push @{$contexts[$ctx]->{input}}, colour( @position );
...
my $painted = 0;
$painted += 0+keys(%{$panels{$_}}) for keys(%panels);
say "Painted panels: ", $painted;
